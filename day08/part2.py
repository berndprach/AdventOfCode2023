...
def print_end_node_equations(starting_node, next_node, instructions,
    ...
    loop_start_step, loop_end_step, goal_node_indices = find_loop(
        starting_node, next_node, instructions
    )
    loop_steps = loop_end_step - loop_start_step
    for goal_node_index in goal_node_indices:
        if goal_node_index < loop_start_step:
            print(f"   Single solution at step {goal_node_index}")
        else:
            print(f"   Repeating solution at "
                  f"{goal_node_index} + i * {loop_steps}")

    if with_simplification:
        loop_goal_node_indices = [i for i in goal_node_indices
                                  if i >= loop_start_step]
        goal_index, loop_steps = simplify(loop_goal_node_indices, loop_steps)
        print(f"   Simplified to: {goal_index} + i * {loop_steps}")
...
def simplify(goal_indices, loop_steps):
    if len(goal_indices) == 1:
        return goal_indices[0], loop_steps

    min_index = min(goal_indices)
    new_goals = [min_index]
    ...
    if len(new_goals) == 1:
        return new_goals[0], loop_steps

    length = len(new_goals)
    regular_indices = [i * min_index for i in range(1, length + 1)]
    if loop_steps % length == 0 and new_goals == regular_indices:
        return min_index, loop_steps // length

    raise Exception(f"Unclear how to simplify "
                    f"{goal_indices} and {loop_steps}")
...
def solve(lines: list[str]) -> int:
    ...
    for node in starting_nodes:
        ...
        loop_steps = loop_end_step - loop_start_step
        goal_index, loop_steps = simplify(goal_node_indices, loop_steps)
        assert goal_index == loop_steps
